...
export default function DigestEN() {
  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-semibold">Daily X Digest</h1>
      <p className="text-white/70">
        Generated: <span className="font-mono">{String(digest.generated_at)}</span>
      </p>
      ...
      <div className="grid gap-4">
        {digest.accounts.map((a: any) => (
          <section key={a.username} className="rounded-2xl border border-white/10 bg-white/5 p-6">
            ...
            {a.error ? (
              <p className="mt-3 text-sm text-rose-200">Fetch error: {a.error}</p>
            ) : null}
